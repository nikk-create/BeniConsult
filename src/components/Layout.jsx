    ...
    <div className="flex flex-col min-h-dvh bg-background">
      ...
      <header className="sticky top-0 z-40 bg-white/80 backdrop-blur border-b border-border px-4 h-14 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-heading font-bold text-base text-gray-900">BéniConsult</span>
        </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-border pb-safe">
        <div className="flex items-center justify-around h-16 px-2">
          {NAV.map(({ to, Icon, label }) => {
            ...
            return (
              <NavLink key={to} to={to} onClick={() => { if (active) window.scrollTo({ top:0, behavior:'smooth' }) }}
                ...
                <span className={`text-[10px] font-medium relative z-10 transition-colors ${active ? 'text-primary' : 'text-gray-400'}`}>{label}</span>
              </NavLink>
            )
          })}
        </div>
